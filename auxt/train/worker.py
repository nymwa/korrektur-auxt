...
class WorkerJobScript(JobScript):
    ...
    def make_train_command(self, data_indices):
        ...
        command.arch(
                ...
                encoder_layers = self.config['train'].get('encoder_layers', 6),
                decoder_layers = self.config['train'].get('decoder_layers', 6),
                ffn_embed_dim = self.config['train'].get('ffn_embed_dim', None),
                attention_heads = self.config['train'].get('attention_heads', None))
